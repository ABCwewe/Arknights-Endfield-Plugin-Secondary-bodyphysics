...
python verify_ui_serialization.py
if %errorlevel% neq 0 (
    echo [VERIFY] UI/serialization checks FAILED
    exit /b 1
)

echo.
echo === [2/4] Forbidden hardcode scan (strict reflection) ===
set "VIOL=0"
...
echo.
echo === [4/4] Full Runtime build ===
call build.bat
if %errorlevel% neq 0 (
    echo   [VERIFY] Runtime build FAILED
    exit /b 1
)
